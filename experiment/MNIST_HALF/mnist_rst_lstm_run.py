...
parser.add_argument('--save_dir', type=str, default='checkpoints/mnist_half_rst_lstm')
parser.add_argument('--gen_frm_dir', type=str, default='/mnt/A/meteorological/2500_ref_seq/MNIST_half_rst_lstm_test/')
...
train_data = MNIST_HALF(
    data_type='train',
    data_root=data_root,
)
valid_data = MNIST_HALF(
    data_type='validation',
    data_root=data_root
)
test_set1_data = MNIST_HALF(
    data_type='test_set1',
    data_root=data_root
)
test_set2_data = MNIST_HALF(
    data_type='test_set2',
    data_root=data_root
)
test_set3_data = MNIST_HALF(
    data_type='test_set3',
    data_root=data_root
)
test_set4_data = MNIST_HALF(
    data_type='test_set4',
    data_root=data_root
)
        ...
        for i_batch, batch_data in enumerate(test_loader):
            ...
            if is_save:
                for bat_ind in range(batch_size):
                    ...
                    for t in range(5):
                        cur_save_path = os.path.join(cur_sample_fold,'img_'+str(t*2+11)+'.png')
...
def wrapper_train(model):
    ...
    limit = 3
    best_iter = None
    for itr in range(18, args.max_iterations + 1):
